...
const validationSchema = Yup.object().shape({
    ...
    // charityId: Yup.string().required("الجمعية مطلوبة"), // الحفاظ على التسمية charityId
    ...
    return (
        <div className="fixed inset-0 z-150 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white dark:bg-slate-900 w-full max-w-3xl rounded-[2.5rem] shadow-2xl border border-white/10 relative overflow-hidden flex flex-col max-h-[90vh]">
                <ModalHeader
                    title={isEdit ? "تعديل بيانات المسؤول" : "إضافة مسؤول جديد"}
                    subtitle="إدارة المسؤولين وربطهم بالجمعيات"
                    icon={ShieldCheck}
                    onClose={onClose}
                        ...
                        <Form className="flex flex-col flex-1 min-h-0">
                            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <SelectField
                                        name="charityId" // تم توحيد الاسم مع الـ initialValues
                                        label="الجمعية"
                                        icon={Building2}
                                    >

...
const AdminDetailsModal = ({ isOpen, onClose, data, charities = [] }) => {
    if (!isOpen) return null;

    const charityName =
        charities.find((item) => String(item.id) === String(data?.charityId))?.name || "—";
            ...
            <div className="bg-white dark:bg-slate-900 w-full max-w-4xl rounded-[2.5rem] shadow-2xl border border-white/10 relative overflow-scroll max-h-[90vh] flex flex-col">
                ...
                <div className="p-8">
                    <div className="flex flex-col items-center text-center mb-8">
                        ...
                        <p className="text-sm text-slate-400 mt-1">بيانات المسؤول</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <Item icon={Building2} label="الجمعية" value={charityName} />
                    </div>
                </div>

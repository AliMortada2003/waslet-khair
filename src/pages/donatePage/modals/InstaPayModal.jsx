...
const InstaPayModal = ({
    ...
    const instapayAddress = "Organization@instapay";
    ...
    const handleCopy = async () => {
        try {
            await navigator.clipboard.writeText(instapayAddress);
        } catch (error) {
    ...
    return (
        <PaymentModalShell
            isOpen={isOpen}
            onClose={onClose}
            title="الدفع عبر إنستا باي"
            subtitle="حوّل المبلغ إلى العنوان التالي ثم أدخل مرجع التحويل"
            ...
            <div className="space-y-5">
                ...
                <div className="rounded-[1.5rem] border border-slate-200 dark:border-white/10 p-5">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <div className="flex-1 h-14 rounded-2xl bg-slate-50 dark:bg-slate-800/40 px-4 flex items-center text-lg font-black text-indigo-600 dark:text-indigo-400">
                            {instapayAddress}
                        </div>
                ...
                {submitState === "failed" && (
                    <div className="rounded-2xl bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 p-4 flex items-center gap-3">
                        ...
                        <p className="font-bold text-red-700 dark:text-red-300">
                            من فضلك أدخل مرجع التحويل أولًا أو حاول مرة أخرى.
                        </p>
                    </div>
                )}
            </div>
